...
use crate::util::fmap;
use crate::refinements::z3;
use std::collections::BTreeSet;

pub fn issue_refinement_error<'c>(context: z3::Context, assert: z3::Ast,
    ...
    assert_location: Location<'c>, origin: Location<'c>)
{
    ...
    // infix form
    let sexpr = context.ast_to_string(assert);
    let (sexpr, variables, _) = parse_sexpr(&sexpr).unwrap();
    let assert = format!("`{}`", sexpr_to_string(&sexpr, Some(cache)));

    let definitions = variables.into_iter()
        .filter_map(|variable| get_z3_definition(context, variable, model, cache))
        .collect::<Vec<String>>()
        .join(", ");
...
pub fn z3_expr_to_string<'c>(context: z3::Context, expr: z3::Ast, cache: &ModuleCache<'c>) -> String {
    let sexpr = context.ast_to_string(expr);
    let (sexpr, _, _) = parse_sexpr(&sexpr).unwrap();
    sexpr_to_string(&sexpr, Some(cache))
}

pub fn debug_z3_ast<'c>(context: z3::Context, expr: z3::Ast) -> String {
    let sexpr = context.ast_to_string(expr);
    let (sexpr, _, _) = parse_sexpr(&sexpr).unwrap();
    sexpr_to_string(&sexpr, None)
}
...
fn sexpr_to_string<'c>(expr: &SExpr, cache: Option<&ModuleCache<'c>>) -> String {
    use SExpr::*;
    match expr {
        List(list) => {
