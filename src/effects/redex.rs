...
pub fn simplify(ast: Ast) -> Ast {
    match ast {
        ...
        Ast::Variable(_) => ast,
        Ast::Lambda(lambda) => {
            Ast::Lambda(Lambda { args: lambda.args, body: Box::new(simplify(*lambda.body)), typ: lambda.typ })
        },
        Ast::FunctionCall(call) => match *call.function {
            ...
            function => {
                ...
                Ast::FunctionCall(FunctionCall { function, args, function_type: call.function_type })
            },
        },
        ...
        Ast::MemberAccess(access) => {
            Ast::MemberAccess(MemberAccess { lhs: Box::new(simplify(*access.lhs)), member_index: access.member_index })
        },
...
fn substitute(ast: Ast, substitutions: &[(DefinitionId, &Ast)]) -> Ast {
    match ast {
        ...
        Ast::Variable(var) => substitutions
            .iter()
        ...
        Ast::FunctionCall(call) => Ast::FunctionCall(FunctionCall {
        ...
        Ast::Extern(_) => todo!(),
        Ast::Assignment(_) => todo!(),
        Ast::MemberAccess(access) => Ast::MemberAccess(MemberAccess {
